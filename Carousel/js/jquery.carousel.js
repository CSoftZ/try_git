...
;(function($){
	...
	$.fn.carousel = function(params){
		var numItems = 0;
		var modifiedStep = 0; // Used only when padEmptySpace is true.
		var btnNextPressed = false;
		var btnPrevPressed = false;
		var params = $.extend({
	...
	function goToStep(env, step) {
		...
		if (env.params.padEmptySpace && modifiedStep != 0) {
		//alert("modifiedStep=[" + modifiedStep + "]");
		    env.steps.first = modifiedStep;
		}
//alert("env.steps.first=[" +  env.steps.first + "]");
	...
	function transition(env, step) {
		...
		if (env.params.direction == "vertical") {
			...
			if (env.params.dispItems == 1 && env.params.padEmptySpace) {
			    //alert("btnNextPressed=[" + btnNextPressed + "], btnPrevPressed=[" + btnPrevPressed + "], step=["+step + "]");				    
				if (btnNextPressed && step <= itemsInCarousel) {
				   modifiedStep = itemsInCarousel;
				   step = modifiedStep;
				}
				if (btnPrevPressed && step == (itemsInCarousel - 1)) {
				   modifiedStep = 0;
				   step = modifiedStep;
				}
				/*if (step < (itemsInCarousel - 1)) {
				   modifiedStep = itemsInCarousel;
				   step = modifiedStep;
				}*/
